...
var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
        center: ol.proj.fromLonLat([-5101850.8,-2651483,2]), // Ajuste para o centro desejado
        zoom: 4
    }),
    controls: ol.control.defaults().extend([
        new ol.control.Zoom()  // Controle de zoom para o mapa principal
    ])
});



// Supondo que você tenha uma instância de mapa chamada "map"
map.on('singleclick', function (event) {
    // Captura as coordenadas do clique
    var coordinates = event.coordinate;

    // Obtém a visualização do mapa
    var view = map.getView();
    var resolution = view.getResolution();

    // Calcula o deslocamento (por exemplo, deslocar 100 pixels para baixo)
    var pixelOffset = -200;
    var offsetY = resolution * pixelOffset;

    // Aplica o deslocamento para ajustar o centro do mapa para baixo
    var adjustedCoordinates = [coordinates[0], coordinates[1] - offsetY];

    // Centraliza o mapa nas coordenadas ajustadas
    map.getView().animate({
        center: adjustedCoordinates,
        duration: 500 // Tempo de animação em milissegundos
    });

    // Exibe o popup ocupando a tela inteira
    var popupElement = document.querySelector('.ol-popup');
    popupElement.style.display = 'block'; // Mostra o popup

    // Define o conteúdo do popup (se necessário)
    var popupContent = document.getElementById('popup-content');
    popupContent.innerHTML = 'Você clicou no ponto: ' + coordinates.join(', '); // Exemplo de conteúdo
});


//Coordenadas iniciais
map.getView().fit([-5101850.8,-2651723.0,-5101338.5,-2651483,2], map.getSize());
...
var miniMap = new ol.Map({
    ...
    controls: ol.control.defaults({ zoom: false }) // Desativa o controle de zoom no mini mapa
  });
...
// Substitua o evento 'pointermove' pelo 'singleclick'
